...
export function LocalStatusPill() {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      // A hosted provider is reachable but never "offline-ready" — the
      // Whisper probe below only decides that for local setups.
      if (!isLocalConfig(cfg)) {
        if (!cancelled) { setStatus("cloud"); setTip(`${getProvider(cfg.providerId).label} · ${cfg.model}`); }
        return;
      }
      ...
      if (!cancelled) {
        if (w.ok) { setStatus("offline-ready"); setTip(`Offline-ready · ${cfg.model} + Whisper`); }
        else { setStatus("local-ready"); setTip(`Local LLM ready · ${cfg.model}`); }
      }
    }
    check();
  ...
  const label = status === "checking" ? "Checking…"
    ...
    : status === "local-broken" ? "Backend down"
    : "Local";
  ...
  return (
    <Link to="/connect" title={tip} className="inline-flex">
      <Badge variant={variant} className="cursor-pointer gap-1 text-[11px]">
        ...
        {label}
        {config && status !== "checking" && status !== "local-broken" && (
          <span className="text-muted-foreground/80 hidden sm:inline">· {config.model.split("/").pop()!.split(":")[0].slice(0, 14)}</span>
        )}
      </Badge>
    </Link>
  );
